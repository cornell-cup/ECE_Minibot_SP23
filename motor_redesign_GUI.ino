...
/** Right motor drivers */
int motor0_pwmPinA = 7; // direction pin, LOW for forward, HIGH for backwards CHANGE PIN
int motor0_pwmPinB = 3; // pwm pin, controls voltage signal for speed
int pwm0 = 80;      // initial pwm value (doesn't really matter)

/** Left motor drivers */
int motor1_pwmPinA = 4; // direction pin, HIGH for forward, LOW for backwards CHANGE PIN
int motor1_pwmPinB = 6; // pwm pin
    ...
    if (process){
        ...
        switch(updated) { //function changes the letter value of updated to a command
          case 'F' : //fwd
            ...
            // maybe place PID() here before analog writes
            analogWrite(motor0_pwmPinA, pwm0);
            analogWrite(motor1_pwmPinB, pwm1);
            PID();

            // set motor0 pwm1 to the calc value pwm0
            // set motor1 pwm 2 to calc value pwm1
          ...
          //for left and right might have to swap A and B
          case 'L' : //left
            Serial.println("Left");
            analogWrite(motor0_pwmPinA, 0);
